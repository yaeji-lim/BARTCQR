...
void getsuff_new(tree& x, tree::tree_cp nx, size_t v, size_t c, xinfo& xi, dinfo& di, sinfo& sl, sinfo& sr,  arma::vec GAMMA_QUANT,
                 arma::vec TAU_SQ_QUANT)
{
  sl.n = 0.0; sl.sum_v_inv = 0.0; sl.sum_y = 0.0; sl.sum_y_div_v= 0.0;
  sr.n = 0.0; sr.sum_v_inv = 0.0; sr.sum_y = 0.0; sr.sum_y_div_v= 0.0;
  ...
  for(size_t i=0;i<di.n;i++) {
    if(nx==x.bn(i, di,xi)) { //does the bottom node = xx's bottom node
      if(di.x(i,v) < xi[v][c]) {
        sl.n++;
        sl.sum_y += di.y(i);
        sl.sum_v_inv += (double)(1.0/(di.vv(i)*TAU_SQ_QUANT(i)) );
        sl.sum_y_div_v += (double)(di.y(i)/(di.vv(i)*TAU_SQ_QUANT(i))  ) ;
      } else {
        sr.n++;
        sr.sum_y += di.y(i);
        sr.sum_v_inv += (double)(1.0/(di.vv(i)*TAU_SQ_QUANT(i)) );
        sr.sum_y_div_v += (double)(di.y(i)/(di.vv(i)*TAU_SQ_QUANT(i))  );
      }
    }
  }
...
void getsuff_new(tree& x, tree::tree_cp nl, tree::tree_cp nr, xinfo& xi, dinfo& di, sinfo& sl, sinfo& sr, arma::vec GAMMA_QUANT,
                 arma::vec TAU_SQ_QUANT)
{
  sl.n = 0.0; sl.sum_v_inv = 0.0; sl.sum_y = 0.0; sl.sum_y_div_v= 0.0;
  sr.n = 0.0; sr.sum_v_inv = 0.0; sr.sum_y = 0.0; sr.sum_y_div_v= 0.0;
...
void getpertsuff_new(tree& x, tree::tree_p pertnode, tree::npv& bnv, size_t oldc, xinfo& xi, dinfo& di, 
                 std::vector<sinfo>& svold, std::vector<sinfo>& svnew,  arma::vec GAMMA_QUANT,
                 arma::vec TAU_SQ_QUANT)
{
	allsuff_new(x,xi,di,bnv,svnew, GAMMA_QUANT, TAU_SQ_QUANT);
	pertnode->setc(oldc);
	allsuff_new(x,xi,di,bnv,svold, GAMMA_QUANT, TAU_SQ_QUANT);		
}
void getpertsuff_new(tree& x, tree::tree_p pertnode, tree::npv& bnv, size_t oldc, size_t oldvar, bool didswaplr, xinfo& xi,
                 dinfo& di, std::vector<sinfo>& svold, std::vector<sinfo>& svnew,  arma::vec GAMMA_QUANT,
                 arma::vec TAU_SQ_QUANT)
{
  allsuff_new(x,xi,di,bnv,svnew, GAMMA_QUANT, TAU_SQ_QUANT);
...
double lil_new(double sum_y_div_v, double sum_v_inv, size_t n, pinfo& pi)
{
  double val1, val2_num, val2_den, rv,sig2;
  sig2 = pi.sigma0*pi.sigma0;
  val1 = 0.5* (log(sig2*pi.phi) - log(pi.phi + sig2*sum_v_inv));
  val2_num = sig2*pow(sum_y_div_v, 2);
  val2_den = 2.0*pi.phi*(pi.phi + sum_v_inv*sig2);
  rv = val1 + val2_num/val2_den;
...
void drmu_new(tree& t, xinfo& xi, dinfo& di, pinfo& pi, arma::vec GAMMA_QUANT,
              arma::vec TAU_SQ_QUANT)
{
  tree::npv bnv;
  std::vector<sinfo> sv;
  allsuff_new(t,xi,di,bnv,sv,GAMMA_QUANT, TAU_SQ_QUANT);
  double theta_star, sigma_star, mu_star;
  for(tree::npv::size_type i=0;i!=bnv.size();i++) { //i th terminal node
    
    // Get the corresponding element from GAMMA_QUANT and TAU_SQ_QUANT for the i-th terminal node
    
    theta_star = pi.sigma0 * pi.sigma0 * (sv[i].sum_y_div_v) /
      (sv[i].sum_v_inv * pi.sigma0 * pi.sigma0 +  pi.phi);
    sigma_star = pi.sigma0 * sqrt( pi.phi) /
      sqrt(sv[i].sum_v_inv * pi.sigma0 * pi.sigma0 +  pi.phi);
    
    
    mu_star = theta_star + R::rnorm(0,1)*sigma_star;
    bnv[i]->setm(mu_star);
    
   // Rcpp::Rcout << "drmu_new " << mu_star<< std::endl;
...
//get sufficients stats for all bottom nodes
void allsuff_new(tree& x, xinfo& xi, dinfo& di, tree::npv& bnv, std::vector<sinfo>& sv, arma::vec GAMMA_QUANT,
                 arma::vec TAU_SQ_QUANT)
{
  tree::tree_cp tbn; //the pointer to the bottom node for the current observations
  size_t ni;         //the  index into vector of the current bottom node
  ...
  for(size_t i=0;i<di.n;i++) {
    ...
    ++(sv[ni].n);
    sv[ni].sum_y += di.y(i);
    sv[ni].sum_v_inv += (double)(1.0/(di.vv(i)*TAU_SQ_QUANT(i)) );
    sv[ni].sum_y_div_v += (double)(di.y(i)/(di.vv(i)*TAU_SQ_QUANT(i)) );
...
bool calirotp_new(tree::tree_p tnew, tree& x, xinfo& xi, dinfo& di, pinfo& pi,arma::vec GAMMA_QUANT,
                  arma::vec TAU_SQ_QUANT, size_t minperbot)
{
  ...
  allsuff_new(x,xi,di,nbold,sold, GAMMA_QUANT, TAU_SQ_QUANT);
  allsuff_new(*tnew,xi,di,nbnew,snew, GAMMA_QUANT, TAU_SQ_QUANT);
  for(size_t i=0;i<nbold.size();i++) {
 
    lilold += lil_new(sold[i].sum_y_div_v, sold[i].sum_v_inv, sold[i].n, pi); 
  }
  for(size_t i=0;i<nbnew.size();i++) {
    if( snew[i].n >= minperbot ) {
      ...
      lilnew += lil_new(snew[i].sum_y_div_v, snew[i].sum_v_inv, snew[i].n, pi); 
    }
...
void calichgv_new(tree& x, xinfo& xi, dinfo& di, pinfo& pi, arma::vec GAMMA_QUANT,
              arma::vec TAU_SQ_QUANT, size_t minperbot)
{
  ...
  for(size_t k=0;k<nognds.size();k++) {
    if(unif_rand()<0.5) //50% of the time we do change of variable, otherwise perturb.
    {
      ...
      tree::npv bnv;
      getpertsuff_new(x,pertnode,bnv,oldcut,oldvar,didswap,xi,di,svold,svnew,  GAMMA_QUANT,TAU_SQ_QUANT);
      ...
      for(bvsz j=0;j!=svold.size();j++) {
        ...
        lilold += lil_new(svold[j].sum_y_div_v, svold[j].sum_v_inv, svold[j].n, pi); 
      }
      ...
      for(bvsz j=0;j!=svnew.size();j++) {
        if( (svnew[j].n) < minperbot)
          ...
          lilnew += lil_new(svnew[j].sum_y_div_v, svnew[j].sum_v_inv, svnew[j].n, pi); 
      }
    ...
    else  //do a perturb proposal
    {
      ...
      std::vector<sinfo> svnew;
      tree::npv bnv;
      getpertsuff_new(x,pertnode,bnv,oldc,pertnode->getv(),didswap,xi,di,svold,svnew, GAMMA_QUANT,TAU_SQ_QUANT);
      ...
      for(bvsz j=0;j!=svold.size();j++) {
        ...
        double gamma_quant = GAMMA_QUANT[j];
        double tau_sq_quant = TAU_SQ_QUANT[j];
      ...
      for(bvsz j=0;j!=svnew.size();j++) {
        if( (svnew[j].n) < minperbot)
          ...
          lilnew += lil(svnew[j].sum_y_div_v, svnew[j].sum_v_inv, svnew[j].n, pi, GAMMA_QUANT[j],  TAU_SQ_QUANT[j]); 
      }
