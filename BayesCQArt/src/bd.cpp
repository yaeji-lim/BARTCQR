...
//      Rcpp::Rcout << "chosen variable and cutpoint are: " << v << ", " << xi[v][c] << std::endl;
    ...
    double Pnogy; //death prob of choosing the nog node at y
      ...
      getsuff_new(x,nx,v,c,xi,di,sl,sr, GAMMA_QUANT, TAU_SQ_QUANT);
       //--------------------------------------------------
      //compute alpha
	  ...
	  if((sl.n>=min_obs_node) && (sr.n>=min_obs_node)) { 
	  	...
	  	lill = lil_new(sl.sum_y_div_v, sl.sum_v_inv, sl.n, pi);  
		lilr = lil_new(sr.sum_y_div_v, sr.sum_v_inv, sr.n, pi); 	
		lilt = lil_new(sl.sum_y_div_v + sr.sum_y_div_v, 
					sl.sum_v_inv + sr.sum_v_inv, 	sl.n + sr.n, pi);  
        ...
        theta_star_left = pi.sigma0*pi.sigma0*(sl.sum_y_div_v)/
						 (sl.sum_v_inv*pi.sigma0*pi.sigma0 + pi.phi);
		sigma_star_left = pi.sigma0*sqrt(pi.phi)/
						sqrt(sl.sum_v_inv*pi.sigma0*pi.sigma0 + pi.phi);

		theta_star_right = pi.sigma0*pi.sigma0*(sr.sum_y_div_v)/
						 (sr.sum_v_inv*pi.sigma0*pi.sigma0 + pi.phi);
		sigma_star_right = pi.sigma0*sqrt(pi.phi)/
						sqrt(sr.sum_v_inv*pi.sigma0*pi.sigma0 + pi.phi);
...
//--------------------------------------------------
      ...
      sinfo sl,sr; 
        ...
        getsuff_new(x,nx->getl(),nx->getr(),xi,di,sl,sr, GAMMA_QUANT, TAU_SQ_QUANT);
          //compute alpha
		  
		double lill = lil_new(sl.sum_y_div_v, sl.sum_v_inv, sl.n, pi);  
		double lilr = lil_new(sr.sum_y_div_v, sr.sum_v_inv, sr.n, pi); 	
		double lilt = lil_new(sl.sum_y_div_v + sr.sum_y_div_v, 
					sl.sum_v_inv + sr.sum_v_inv, 	sl.n + sr.n, pi);  
    ...
    //metrop
		...
		theta_star_nog = pi.sigma0*pi.sigma0*(sum_y_div_v_nog)/
						 (sum_v_inv_nog*pi.sigma0*pi.sigma0 + pi.phi);
		sigma_star_nog = pi.sigma0*sqrt(pi.phi)/
						sqrt(sum_v_inv_nog*pi.sigma0*pi.sigma0 + pi.phi);
