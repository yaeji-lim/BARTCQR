...
// [[Rcpp::export]]
Rcpp::List BayesCQArt(arma::vec const& y,
                     arma::mat const& X, 
...
//  vec b_c = b_k.elem(conv_to<uvec>::from(zi_c - 1));
  ...
  arma::vec ycentered = y - (ysorted(floor(0.5*n)) );
...
//Rcpp::Rcout << "For each tree : " << j<< std::endl;
 ...
 // Rcpp::Rcout << "Done for alpha" << temp_alpha<< std::endl;
  ...
  return Rcpp::List::create(
    Rcpp::Named("pred_train")=(ysorted(floor(0.5*n)) ) + arma::min(ycentered) + (pmean + 0.5)*(arma::max(ycentered) - arma::min(ycentered)), 
    Rcpp::Named("pred_test") = (ysorted(floor(0.5*n)) ) + arma::min(ycentered) + (ppredmean + 0.5)*(arma::max(ycentered) - arma::min(ycentered)), 
